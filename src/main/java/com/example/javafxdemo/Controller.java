...
public class Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        colCliente.setCellValueFactory(data-> new SimpleStringProperty(data.getValue().getCliente().getNome()));
...
/workspaces/atividade-pratica/target
    ...
    protected void novaConta() {
        ...
        novaConta.getCliente().setNome(txCliente.getText());
        if(!txSaldo.getText().isEmpty()){
            novaConta.depositar(Double.valueOf(txSaldo.getText()));
        }
    ...
    protected void sacar() {
        contaOperacao().ifPresent(conta -> conta.sacar(valorOperacao().orElse(0d)));
        tbContas.refresh();
    }
    @FXML
    protected void depositar() {
        contaOperacao().ifPresent(conta -> conta.depositar(valorOperacao().orElse(0d)));
        tbContas.refresh();
    }
